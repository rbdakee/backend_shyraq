...
- **`schedule:weekly-rollout` cron lock (B7 T5 leftover)** — single-process app сейчас, поэтому cron безопасен. Когда в B9 поднимется horizontal-scale на api-инстансы — нужен distributed lock (Redis `SET NX EX` или BullMQ scheduled-job). Маркеры `// TODO(B9): move cron to dedicated worker process when BullMQ split lands` и `// TODO(B9): add cron lock via Redis SET NX EX once the API horizontally scales` стоят в `src/modules/schedule-rollout/weekly-rollout.cron.ts`.
...
- **NotificationPort post-commit microtask vs real afterCommit (B8 leftover)** — `Promise.resolve().then(...)` fires on a microtask before TypeORM commits. Safe with current sync `LoggingNotificationAdapter` but unsafe for B9 WS fanout (broadcast for rolled-back row). Resolve in B9 by either attaching to `queryRunner.afterCommit` or introducing an Outbox table. Marker `// TODO(B9): real afterCommit / Outbox` in `attendance.service.ts` and `timeline.service.ts` near the dispatch sites.
