...
**Текущее состояние (B8):** 51 unit suites / 569 tests green. Integration (INTEGRATION_DB=1): 61/62 suites / 582/583 tests (1 pre-existing flaky `enrollment.service.integration.spec.ts:323` — not a B8 regression). E2E: 14 suites / 111 tests. Build + lint: green. Endpoints под `/api/v1/...`. Swagger полный.
...
- **`autoApprovePendingPrimaries` без notify (B6 leftover)** — ~~план §3.3 step 4 предусматривал `notifyGuardianApproved` self-event при auto-approve. T4 решил не эмитить (self-event пока без подписчика на стороне UI). Когда в B9 заведём WebSocket-роутинг — добавить вызов `notificationPort.notifyGuardianApproved` внутри per-kg tx. Маркер `// TODO(B9): emit notifyGuardianApproved` стоит в `src/modules/auth/auth.service.ts` в `autoApprovePendingPrimaries`.~~ **Resolved in B9 T8**: `notifyGuardianApproved` emitted synchronously inside per-kg TX in `autoApprovePendingPrimaries`.

- **`schedule:weekly-rollout` cron lock (B7 T5 leftover)** — ~~Маркеры `// TODO(B9)` стояли в `weekly-rollout.cron.ts`.~~ **Resolved in B9 T6**: `weekly-rollout.cron.ts` deleted; WeeklyRolloutProcessor migrated to BullMQ repeatable-job with Redis-based distributed lock.
...
- **NotificationPort post-commit microtask vs real afterCommit (B8 leftover)** — ~~`Promise.resolve().then(...)` fires on a microtask before TypeORM commits. Marker `// TODO(B9): real afterCommit / Outbox` in `attendance.service.ts` and `timeline.service.ts`.~~ **Resolved in B9 T8**: all `fireAndForget` wrappers replaced with synchronous `await notifyXyz(...)` inside the same ambient TX; `OutboxNotificationAdapter` writes atomically via tenantStorage EntityManager.
