...
**Текущее состояние (B7):** 49 unit suites / 527 tests green (13 suites / 56 tests skipped). Integration (INTEGRATION_DB=1): 61/62 suites / 582/583 tests (1 pre-existing flaky `enrollment.service.integration.spec.ts:323` ordering check — not a B7 regression). E2E: 12 suites / 90 tests. Build + lint: green. Endpoints под `/api/v1/...`. Swagger полный.
