...
- [x] **B10** — Identity QR ✓ **demo-ready: BP §13** (commit `75c27b0`)
...
- [x] **BP §13** Identity QR System *(B10: GET /users/me/qr, POST /staff/qr/scan, POST /admin/qr/revoke-all/:userId — cross-tenant token, kg-scoped linkedChildren, advisory-lock-protected issue, rate-limit 60/min/device)*
...
- **`POST /auth/role/select` RLS bug for parent path (B10 T7 finding, pre-existing)** — `selectRole` for parent → triggers refresh-token RLS violation because no `app.kindergarten_id` GUC is set in the surrounding TX (refresh-token row gets a `kindergarten_id`, but no `KindergartenScopeGuard` engaged). Worked around in B10 e2e Scenario K via direct-DB seed; no parent multi-role flow currently exercised in production. **Fix in B22 polish or whoever next touches auth role-select.** Bug is in `src/modules/auth/auth.service.ts` `selectRole`.
- **Advisory lock bypass when no ambient TX (B10 T7 observation)** — `IdentityQrService.issueOrRefresh` advisory lock `pg_advisory_xact_lock(hashtext('qr:identity:'||userId))` relies on the ambient `TenantContextInterceptor` TX; CLI/worker callers with no ambient TX would acquire the lock only for the duration of their own explicit TX or not at all. No such caller exists today. Reconsider if a future migration/worker invokes `issueOrRefresh`. Comment noted in code.
