...
| Метод | Путь | Назначение |
|---|---|---|
| POST | `/admin/qr/revoke-all/:userId` | Auth: `JwtAuthGuard` + `KindergartenScopeGuard` + `AdminScope`. Bulk-revoke всех активных токенов пользователя: DB `revoked_at = NOW()` на всех `user_qr_tokens WHERE user_id=:userId AND revoked_at IS NULL` + Redis DEL для каждого токена. Security-recourse при подозрении на компрометацию QR. Response: `{revoked_count: number}`. 404 `user_not_found` если userId не существует. |
...
| Метод | Путь | Назначение |
|---|---|---|
| POST | `/staff/qr/scan` | Сканирование QR пользователя. Auth: `JwtAuthGuard` + `KindergartenScopeGuard` + `StaffOnly`. Body: `{token: string}` (32 hex). 1) Rate-limit check: 60/мин per `device_id` (берётся из активной `refresh_tokens` сессии вызывающего staff; Redis `rl:qr:scan:{device_id}` INCR+EXPIRE 60); 2) Redis lookup `qr:token:{plaintext}` → `user_id` (miss → 404); 3) DB load `user`; если parent — load `linked_children[]` из approved guardians (`{id, full_name, current_group_id, photo_url}`); 4) Вычисление `allowed_actions`: parent → `['check_in','check_out']` при наличии approved guardian с `can_pickup=true` иначе `[]`; staff → `['gate_entry']`; super-admin → `[]`; 5) update `last_scanned_at`; 6) return `{user, linked_children?, allowed_actions}` (`linked_children` только для role=parent). Errors: 404 `qr_token_not_found`, 410 `qr_token_expired`, 410 `qr_token_revoked`, 429 `qr_rate_limit_exceeded`. |
