...
| Метод | Путь | Назначение |
|---|---|---|
| POST | `/push-tokens` | Зарегистрировать device token. Body: `token` (string), `platform` (`'ios'`/`'android'`/`'web'`), `app_version?` (string), `device_id?` (string). Upsert по `(user_id, token)`, `is_active=true`. Используется Parent App и Staff App. |
...
**Auth:** `WsJwtGuard` — обёртка над `JwtTokenPort`, проверяет токен на handshake. Неверный/отсутствующий токен → disconnect `401`.

**Auto-subscribe (без client-subscribe message):** после успешного handshake `NotificationGateway` автоматически добавляет сокет в комнаты:
- `user:{user_id}` — всегда.
- `child:{child_id}` — для каждого ребёнка из `child_guardians WHERE user_id=? AND status='approved' AND revoked_at IS NULL`.
- `group:{group_id}` — для каждой группы из `group_mentors WHERE user_id=? AND is_active=true`.
