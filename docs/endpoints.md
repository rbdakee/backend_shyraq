...
| POST | `/auth/otp/verify` | Проверить OTP. Headers: `X-Device-Id` (optional) — стабильный per-install device id; персистится на новом `refresh_tokens.device_id` (NULL для legacy-клиентов). Читает `otp:login:{phone}` (missing → 400 `otp_expired_or_missing`); mismatch → `HINCRBY attempts`; at 3 → `SET otp:locked:{phone} TTL 900s` + `DEL otp:login:{phone}` → 429 `otp_locked`. Success → DEL, lookup/create user by phone. Выдача токенов зависит от ролевого резолва (см. ниже). |
| POST | `/auth/refresh` | Ротация refresh-токена. Headers: `X-Device-Id` (optional) — перезаписывает `device_id` на свежей строке; legacy-клиенты не передают, и старое значение с предыдущей строки переносится. Body `{refresh_token}`. Lookup по `SHA256(refresh_token)`; проверка `revoked_at IS NULL AND expires_at > NOW()` (иначе 401 `invalid_refresh`). В одной TypeORM-транзакции: `UPDATE refresh_tokens SET revoked_at=NOW()` + `INSERT` новый; после commit — `SET token:blocklist:{old_access_jti}` с TTL = remaining access TTL (best-effort). Response — новая пара `{access_token, refresh_token}`. Клиент должен передать текущий access-токен в `Authorization: Bearer …` вместе с body refresh_token — без него blocklist пропускается (best-effort). |
...
| GET | `/users/me/qr` | (B10) Возвращает текущий Identity QR. Auth: `JwtAuthGuard` (любая роль кроме `pending_role_select`). Авто-refresh: если активного нет или до `expires_at` осталось <1 часа — транзакционно ревокирует старый (`user_qr_tokens.revoked_at`), создаёт новый (TTL 24ч), синхронизирует Redis `qr:token:{plaintext}`. Ответ: `{token: string (32 hex chars), issued_at: ISO8601, expires_at: ISO8601}`. Lazy-issue: токен создаётся только при первом вызове, не на login. |
...
| Метод | Путь | Назначение |
|---|---|---|
| POST | `/admin/qr/revoke-all/:userId` | Auth: `JwtAuthGuard` + `KindergartenScopeGuard` + `AdminScope`. Bulk-revoke всех активных токенов пользователя: DB `revoked_at = NOW()` для всех активных строк `user_qr_tokens WHERE user_id=:userId AND revoked_at IS NULL`; Redis-записи **не удаляются** (admin не имеет plaintext-токенов, только их хэши); cache-TTL ≤24h задаёт верхнюю границу stale-cache exposure; следующий скан попадает в DB-recheck и возвращает 410 `qr_token_revoked`. Security-recourse при подозрении на компрометацию QR. Response: `{revoked_count: number}`. 404 `user_not_found` если userId не существует. |
