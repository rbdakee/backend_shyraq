...
| POST | `/admin/kaspi/connect/verify-otp` | Body: `{process_id, otp: '1234'}` — Kaspi присылает **4-значный** код (валидатор терпит 4–6 цифр). Подтверждает код → авто-`finish` (ECDH-обмен → `vtokenSecret`) → org-context → сохраняет `kaspi_merchant_session` (всё чувствительное enc), `status=active`. Response 200: `{connected: true, phone, org_name, profile_id}`. Errors: 400 `kaspi_unknown_process`, 401 `kaspi_otp_invalid`, 409 `kaspi_no_business_profile` (устройство зарегистрировалось, но у аккаунта нет мерчант-профиля), 409 `kaspi_device_verification_required` (код принят, но Kaspi требует селфи-верификацию Kaspi ID нового устройства — у бэкенда нет камеры), 502 `kaspi_finish_failed`. |
...
**Errors (§2.25):** 401, 403 `forbidden`, 400 `kaspi_unknown_process`, 400 `kaspi_invalid_phone`, 401 `kaspi_otp_invalid`, 409 `kaspi_already_connected`, 409 `kaspi_password_login_required`, 409 `kaspi_no_business_profile`, 409 `kaspi_session_taken_over`, 409 `kaspi_device_verification_required`, 401 `kaspi_password_invalid`, 404 `kaspi_not_connected`, 422 `invalid_phone_format`, 502 `kaspi_app_version_outdated` / `kaspi_finish_failed`, 429.
