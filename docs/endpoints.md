...
| Метод | Путь | Назначение |
|---|---|---|
| POST | `/auth/otp/request` | Отправить OTP. Body `{phone, app}` (`app` ∈ `parent`/`staff`/`admin`). Валидация phone `/^\+7\d{10}$/`. **`app=staff`/`admin`: closed-app** — ищем `users` по phone + активную `staff_members` с допустимой ролью (см. таблицу audience); нет → **404 `not_invited`, OTP НЕ отправляем** (staff/admin заводятся только инвайтом из админки/суперадмина). **`app=parent`: open registration** — OTP шлём всегда. Rate-limit 5/hour per phone (Redis `rate:otp:{phone}` TTL 3600с). Lockout-check: `otp:locked:{phone}` → 429 `otp_locked`. Генерит 6-digit код (или `OTP_TEST_CODE` для phone'ов из `TEST_PHONES`), пишет `otp:login:{phone}` Hash `{code, attempts:0}` TTL 300с. SMS через `SmsProvider` (mock-adapter логирует в stdout; для whitelisted phone'ов не шлётся). Response `202 Accepted` — `{sent: true, registered: <bool>, resend_after_sec: 60}` (`registered` = существует ли уже `users`-строка; для staff/admin всегда `true`). |
...
| Метод | Путь | Назначение |
|---|---|---|
| POST | `/saas/billing/monthly-run` | Ручной триггер ежемесячной генерации инвойсов. Body: `{period_start?: '2026-06-01'}` — без него берётся первое число текущего месяца (Almaty). **Всегда cross-tenant:** обходит все неархивированные садики; `kindergarten_id` в body → 400 `single_kg_trigger_not_supported_b13` (per-kg триггер отложен до B22). **Асинхронный:** ставит job в BullMQ и сразу отвечает **202** `{job_id, status: 'enqueued'}` — итог смотреть в логах воркера или запросом инвойсов, тела с counters нет. Идемпотентен: advisory lock per `(kg_id, period_start)` + short-circuit `existsMonthlyForPeriod`, который считает **только** `invoice_type='monthly'` — предоплатные и one-off строки с тем же `period_start` прогон не блокируют. Дети с покрывающей **оплаченной** предоплатой пропускаются пер-ребёнку. |
...
### 1.9 BCC e-Commerce — merchant onboarding (Super-Admin)

**Auth:** `@SuperAdminScope()` (bypass RLS). BCC-реквизиты принадлежат конкретному садику и хранятся в `bcc_merchant_accounts`; tenant-admin не получает MAC key или callback credentials.

| Метод | Путь | Назначение |
|---|---|---|
| PUT | `/saas/kindergartens/:kindergartenId/bcc/account` | Создать либо обновить draft account. Body: `{merchant_id, terminal_id, merchant_name?, environment: 'test'\|'live', mac_key_component_1, mac_key_component_2}`. Компоненты — HEX одинаковой длины; backend выполняет XOR, шифрует только итоговый MAC key через `CryptoCipherPort` и не сохраняет компоненты. При первом создании генерирует callback token и Basic Auth credentials. Для server-owned `NOTIFY_URL` сохраняются SHA-256 token hash и отдельный AES-GCM ciphertext token; plaintext не сохраняется. Response 200: публичный status account + **одноразово** `{notify_url, notify_username, notify_password}` для регистрации у BCC; MAC и его компоненты никогда не возвращаются. Errors: 404 `kindergarten_not_found`, 409 `bcc_account_active` (для смены активного ключа использовать rotation), 422 `bcc_mac_components_invalid`. |
| GET | `/saas/kindergartens/:kindergartenId/bcc/account` | Статус без секретов: `{connected, status: 'draft'\|'active'\|'disabled', merchant_id, terminal_id, merchant_name?, environment, last_connection_checked_at?, last_connection_result?}`. `last_connection_result` содержит только безопасные result codes. Errors: 404 `bcc_account_not_found`. |
| POST | `/saas/kindergartens/:kindergartenId/bcc/account/check` | Выполнить `TRTYPE=800`. Успех активирует account; ошибка оставляет его `draft` и сохраняет санитизированный результат. Response 200: `{connected, status, checked_at, result}`. Errors: 404 `bcc_account_not_found`, 502 `bcc_gateway_unavailable` / `bcc_connection_check_failed`. |
| POST | `/saas/kindergartens/:kindergartenId/bcc/account/disable` | Запретить новые BCC-платежи (`status='disabled'`) без удаления истории. Callback, reconciliation и refund ранее созданных платежей продолжают работать. Response 200: `{status: 'disabled'}`. |
| POST | `/saas/kindergartens/:kindergartenId/bcc/account/rotate-mac` | Body: `{mac_key_component_1, mac_key_component_2}`. Сохраняет только новый зашифрованный XOR-результат, переводит account в `draft`; для возврата в `active` обязателен повторный connection check. |
| POST | `/saas/kindergartens/:kindergartenId/bcc/account/rotate-callback-credentials` | Ротирует callback token и Basic password. Response 200 содержит новые `{notify_url, notify_username, notify_password}` только один раз; старый callback URL сразу перестаёт приниматься. |

`notify_url` для dev: `https://balam-api-dev.innodev.kz:443/api/v1/webhooks/payments/bcc/{callbackToken}`. Для BCC support передаются URL, явный port `443`, метод `POST`, Basic Auth, TLS 1.2 и virtual host. До получения production credentials интеграция проходит применимые банковские test cases на опубликованных BCC test TID/MAC/cards и отправляет санитизированный отчёт в согласованный чат. Только после approval банк выдаёт production TID и две MAC components. Production account активируется после отдельного production connection check; вопрос «одна сертификация платформы или каждого tenant» уточняется перед production onboarding.
...
| POST | `/admin/schedule/week-snapshots/rebuild` | Пересобрать уже материализованные недели из текущего определения шаблона. Body: `{group_id?}` (опущен → все группы). Response: `{rebuilt_weeks, deleted_events, inserted_events}`. Вызывается автоматически при любой мутации шаблона; ручной запуск нужен для разового ремонта данных. Затрагивает **текущую и будущие** недели, у которых уже есть snapshot, и только события `origin='template'` со `status='scheduled'` и `starts_at > now()`. Сохраняет: прошлое, тронутое персоналом, ad-hoc. Новые snapshot'ы не создаёт — горизонт материализации остаётся за cron'ом. |
| GET | `/admin/schedule/activity-events` | Список `activity_events`. Query: `group_id?`, `date_from`, `date_to`, `status?`. Response: `[{id, group_id, template_slot_id?, origin, activity_name, category, location_id?, location_name?, starts_at, ends_at?, status, created_by?, notes?}]`. `category` — enum `lesson|activity|meal|sleep` (копируется из слота при проекции; ad-hoc → серверный default `'activity'`, принимается опционально в POST/PATCH `activity-events`). `origin` — `template|adhoc`, immutable; **`template_slot_id = null` НЕ означает ad-hoc** (FK обнуляется при удалении слота) — происхождение определяй только по `origin`. `location_name` — computed display-overlay (`locationId → locations.name`); `null`, если `location_id` пуст / локация не найдена / имя пустое. |
...
| POST | `/admin/invoices/:id/manual-mark-paid` | Ручная отметка оплаты наличкой. Body: `{paid_at?, payer_user_id?, note?, amount?}`. Создаёт `payments` с `provider='cash'`, `status='completed'`. `amount` опционален (зеркалит `payment_mode=partial` из `/payments/initiate`): не передан или == остатку → полное погашение (invoice → `paid`); `0 < amount < остаток` → частичный кэш-платёж (invoice → `partial`, событие `invoice.paid` не эмитится); `amount <= 0` или `> остатка` → 409 `invoice_status_invalid` (`amount_mismatch_partial`). **Частичный кэш недопустим на предоплатном счёте:** для `invoice_type='prepayment_*'` любое `amount` меньше остатка → 400 `prepayment_partial_not_allowed` (тот же инвариант, что и на родительском `/pay`); передать полный остаток можно. Conditional UPDATE WHERE status IN ('pending','partial','overdue') RETURNING *; 409 `invoice_already_paid` при race. Response 200: invoice object (`amount_paid`/`amount_remaining` пересчитаны). Errors: 400 `prepayment_partial_not_allowed`, 404 `invoice_not_found`, 409 `invoice_already_paid`, 409 `invoice_status_invalid`. |
...
<!-- Ведение журнала из админки (check-in/out, daily-status, soft-delete, history) + модуль `audit`; DDL — миграция AdminAttendanceAudit. -->

**Guards:** `JwtAuthGuard, PendingRoleSelectGuard, RolesGuard` + `@Roles('admin','reception')` на весь раздел. Маршруты дублируют `/staff/attendance/*` по путям, но делегируют в **тот же** `AttendanceService` — второй реализации check-in нет. Разница только в правах: у админа нет edit-window на PATCH (staff ограничен тем же календарным днём Asia/Almaty), правки `childId`/`eventType` и DELETE — admin-only, back-dated записи — молчаливые.

**Display-overlays (B-N2):** `attendance_event` несёт computed `recorded_by_full_name` (`recordedBy → staff_members.full_name ?? users.full_name`), `pickup_user_full_name` (`pickupUserId → users.full_name`, `null` на check-in / без pickup) и `child_name` (`childId → children.full_name`, включая архивных); `child_daily_status` несёт `set_by_full_name` (`setBy → staff_members.full_name ?? users.full_name`); запись истории — `actor_full_name` (`actorStaffId → staff_members.full_name ?? users.full_name`). Все — `null` при пустом источнике / не найдено / имя пустое. Резолв батчем в сервисе (без N+1).

**QR — это два шага.** `POST /admin/qr/scan` (§2.23) **только резолвит личность и посещаемость НЕ пишет** — единственная его запись — `last_scanned_at` на самом токене. Чтобы зафиксировать проход, панель делает **отдельный** вызов `check-in`/`check-out` с `childId`, взятым из `linked_children` ответа скана. Скан без второго вызова не оставляет следа в журнале.

**Back-dated записи — молчаливые.** `recordedAt` на календарный день, отличный от сегодняшнего (Asia/Almaty) ⇒ push родителю **не уходит**; событие, timeline-entry и audit-entry пишутся как обычно. Живая запись (`recordedAt` не передан или сегодняшний) уведомляет как раньше. Смысл: админ, закрывающий вчерашний журнал в 22:00, не должен слать родителям «ребёнок пришёл». `recordedAt` больше чем на 5 минут в будущем (допуск на расхождение часов) → 422 `invalid_attendance_timestamp`.

**Корректировки `childId` / `eventType` — admin-only и каскадные.** `childId` перевешивает событие на другого ребёнка (запись оформили не на того), `eventType` переключает `check_in ⇄ check_out` (промахнулись кнопкой). Каскад: парная timeline-запись переезжает вместе с событием (при флипе типа — пересоздаётся, т.к. `entry_type` неизменяем), а `child_daily_status` пересчитывается для **обоих** (ребёнок, день) — и откуда ушло, и куда встало. На check-out пара (ребёнок, забирающий) перевалидируется → 403 `pickup_user_not_allowed`. `method` **immutable** — он фиксирует, как строка появилась. PATCH молчит (без push). На `/staff/attendance/:eventId` те же поля → 403 `attendance_correction_admin_only`.

**DELETE — soft.** Строка получает `deleted_at` (tombstone), но исчезает из **всех** чтений: лог событий, `GET /:eventId`, таймлайн ребёнка, счётчики дашборда. Парная timeline-запись удаляется, `child_daily_status` пересчитывается. Повторный DELETE → 404 (удалённые читаются как отсутствующие). История в `audit_log` переживает удаление.

**Правило демоушена (при DELETE / PATCH).** `present → absent` — **только** если на этот день не осталось ни одного живого `check_in`. Явные операторские статусы (`sick`, `on_vacation`, `late`, `early_pickup`) не перезаписываются никогда — они выигрывают у выведенного из лога. Обратный ход (`absent|late → present`) — как на check-in. Компромисс: `present`, проставленный руками **без** check-in, неотличим от выведенного, поэтому удаление постороннего события в этот день демоутит его в `absent`.

| Метод | Путь | Назначение |
|---|---|---|
| POST | `/admin/attendance/check-in` | Ручной check-in из панели (после `/admin/qr/scan` либо с `childId`, выбранным руками): `childId`, опц. `recordedAt`, `notes`. `method='manual'` — клиентом не задаётся. Промоутит `child_daily_status` `absent\|late → present`. 201 + событие. |
| POST | `/admin/attendance/check-out` | Ручной check-out: `childId`, `pickupUserId`, опц. `recordedAt`, `notes`. `pickupUserId` обязан быть approved active pickup-guardian ребёнка → иначе 403 `pickup_user_not_allowed`. **Дневной статус НЕ меняет** (intraday-статус двигают только check-in и `POST /admin/daily-status`). 201 + событие. |
| GET | `/admin/attendance-events` | Лог check-in/out. Фильтр: `childId`, `groupId` (резолвится через `children.current_group_id`), `from`/`to` (ISO-8601), `limit` (1–500) / `offset`. Soft-deleted события не отдаются. |
| GET | `/admin/attendance-events/:eventId` | Одно событие. 404 `attendance_event_not_found` (в т.ч. если soft-deleted). |
| PATCH | `/admin/attendance-events/:eventId` | Корректировка: `recordedAt`, `notes`, `pickupUserId` + **admin-only** `childId`, `eventType`. Без edit-window — админ правит событие любой давности. См. преамбулу о каскаде и immutable `method`. |
| DELETE | `/admin/attendance-events/:eventId` | **Soft-delete** ошибочно заведённой записи. `204`. См. преамбулу. |
| GET | `/admin/attendance-events/:eventId/history` | История правок события из `audit_log`, новые первыми: `{id, action('create'\|'update'\|'delete'), actorUserId, actor_full_name, before, after, createdAt}`. Query `limit` (1–500, default 100) / `offset`. `before`/`after` — сырые jsonb-снимки строки (таймстемпы — ISO-строки), фронт диффает сам; `create` несёт только `after`, `delete` — только `before`. Работает и для **soft-deleted** событий. Событие без правок → ровно одна запись `create`. Неизвестный `eventId` → пустой массив (не 404 — история переживает строку). |
| POST | `/admin/daily-status` | Upsert статуса на (`childId`, `date`): `childId`, `date` (YYYY-MM-DD), `status` (`present\|absent\|sick\|late\|early_pickup\|on_vacation`), опц. `note`. **200, не 201** (upsert). Явный статус outranks выведенный: проставленный здесь `sick` не перетрётся ни последующим check-in, ни пересчётом при корректировке. Уведомляет родителя. |
| GET | `/admin/daily-status` | Сводка `child_daily_status`. Фильтр: `childId`, `from`/`to`, `limit`/`offset`. |
| GET | `/admin/daily-status/summary` | Агрегированная сводка отсутствий (для заявок vacation/day_off). **Не реализован** — в коде отсутствует; для сводки дня использовать `/admin/dashboard/attendance-today` (§2.22). |

**Audit.** Каждая мутация раздела (check-in, check-out, PATCH, DELETE, `POST /admin/daily-status`) пишет строку в `audit_log` с актором и before/after-снимками — в той же транзакции, что и сама мутация (откатывается вместе с ней, «призрачных» записей не бывает). Именно поэтому `childId`/`eventType` вообще редактируемы: история живёт в `audit_log`, а не в неизменности строки. Читается через `GET /admin/attendance-events/:eventId/history` (сейчас `entity_type='attendance_event'`; `child_daily_status` пишется, но своего read-роута пока не имеет).

**Error codes (§2.21):** 400 `tenant_required`, 401, 403 `forbidden` / `pickup_user_not_allowed` / `attendance_correction_admin_only` (на staff-роуте) / `attendance_edit_window_expired` (на staff-роуте), 404 `attendance_event_not_found` / `child_not_found` / `staff_member_not_found`, 422 `invalid_attendance_timestamp` / `invalid_attendance_pickup` (pickup на check_in-событии), 429.
...
| Метод | Путь | Назначение |
|---|---|---|
| POST | `/admin/qr/scan` | Алиас `POST /staff/qr/scan` для Admin Web (см. §3.5 — семантика, headers, errors и response идентичны, тот же `IdentityQrService.scan()`). Отличие только в guard: `@Roles('admin','reception')` (method-level, переопределяет class-level `@Roles('admin')` — `revoke-all` ниже остаётся admin-only). Существует чтобы админ-панель не ходила по `/staff/...`-пути; канонический маршрут для Staff App — `/staff/qr/scan`. **Только идентификация — посещаемость не пишет**: для check-in/check-out нужен отдельный последующий вызов. `X-Device-Id` обязателен и сверяется `=` с `device_id` активных `refresh_tokens` вызывающего — клиент, залогинившийся БЕЗ этого header'а, имеет `NULL` в строке, а `NULL = <что угодно>` никогда не истинно → **всегда** 401 `no_active_session_for_device`. Админ-панель обязана слать один и тот же device-id (UUID, сгенерить раз и держать в localStorage) на **обоих INSERT-роутах**: `POST /auth/otp/verify` и `POST /auth/role/select`. Второй — главная ловушка: он делает `refreshTokens.create({deviceId: … ?? null})` (не rotate), и мульти-садиковый админ проходит через него всегда, так что пропуск header'а именно там убивает скан. На `/auth/refresh` слать желательно, но не критично: `rotate` делает `deviceIdOverride ?? existing.device_id` — при отсутствии header'а старое значение переносится. |
...
| POST | `/admin/kaspi/connect/verify-otp` | Body: `{process_id, otp: '1234'}` — Kaspi присылает **4-значный** код (валидатор терпит 4–6 цифр). Подтверждает код → авто-`finish` (ECDH-обмен → `vtokenSecret`) → org-context → сохраняет `kaspi_merchant_session` (всё чувствительное enc), `status=active`. Response 200: `{connected: true, phone, org_name, profile_id}`. Errors: 400 `kaspi_unknown_process`, 401 `kaspi_otp_invalid`, 409 `kaspi_no_business_profile` (устройство зарегистрировалось, но у аккаунта нет мерчант-профиля), 409 `kaspi_device_verification_required` (код принят, но Kaspi требует селфи-верификацию Kaspi ID нового устройства — у бэкенда нет камеры), 409 `kaspi_device_not_authorized` (устройство зарегистрировалось, но самый первый вызов свежевыданным токеном отвергнут `StatusCode=-101001` — аккаунт так и не авторизовал это устройство), 502 `kaspi_finish_failed`. **`kaspi_session_taken_over` онбординг больше не возвращает** — см. врезку. |
...
**Errors (§2.25):** 401, 403 `forbidden`, 400 `kaspi_unknown_process`, 400 `kaspi_invalid_phone`, 401 `kaspi_otp_invalid`, 409 `kaspi_already_connected`, 409 `kaspi_password_login_required`, 409 `kaspi_no_business_profile`, 409 `kaspi_session_taken_over`, 409 `kaspi_device_not_authorized`, 409 `kaspi_device_verification_required`, 401 `kaspi_password_invalid`, 404 `kaspi_not_connected`, 422 `invalid_phone_format`, 502 `kaspi_app_version_outdated` / `kaspi_finish_failed`, 429.

> **Kaspi Pay = одно активное устройство на аккаунт.** Установлено вживую 03.08.2026.
>
...
>
> ⚠️ **`-101001` ≠ доказательство перехвата.** `Description` буквально означает «этот токен не годится»; русский `Message` — шаблонная подсказка, которую Kaspi цепляет к тому же коду. Трактовку задаёт **вызывающая сторона**, а не код:
>
> | Где поймали `-101001` | Что это значит | Код |
> |---|---|---|
> | `sign-in-lite` (`refreshSession`) — токен раньше принимался | перехват: устройство вытеснили | 409 `kaspi_session_taken_over` |
> | `org-context-otp` в `doFinish`, `finish` вернул `x509` — токену секунды, он ни разу не использовался | устройство зарегистрировалось, но аккаунт его не авторизовал | 409 `kaspi_device_not_authorized` |
> | то же, но `finish` **не** вернул `x509` | MAC ушёл заглушкой `'000000'` → запрос фактически не подписан; это дрейф контракта у нас | 502 `finish_org_context_unsigned_vtoken` |
>
> До 03.08.2026 все три случая отдавались как `kaspi_session_taken_over`, и админ на свежем онбординге получал инструкцию искать несуществующий второй вход. `hasX509` теперь пишется в лог рядом с `hasOrgId` (плюс отдельный ERROR `finish returned no x509`), поэтому третья строка отделяется от второй за одну попытку.
>
...
>
> **Коды:** 409 `kaspi_session_taken_over` / 409 `kaspi_device_not_authorized` / 502 `finish_org_context_unsigned_vtoken` (`StatusCode=-101001` — какой именно, см. таблицу выше), 409 `kaspi_password_login_required` (экран пароля на send-phone → идти в send-password), 401 `kaspi_password_invalid` (пароль отвергнут либо блокировка), 409 `kaspi_no_business_profile` (`StatusCode=0` и пустой `Data.Current` — у аккаунта нет мерчант-профиля), 502 `finish_org_context_rejected` (прочие ненулевые `StatusCode`), 502 `send_password_field_unknown` (ни один кандидат не принят — дрейф контракта на нашей стороне).
>
...
| PATCH | `/staff/attendance/:eventId` | Корректировка ранее сделанной записи: `recorded_at`, `notes`, `pickup_user_id`. Окно — тот же календарный день в Asia/Almaty, иначе 403 `attendance_edit_window_expired`. Структурные правки (`childId`, `eventType`) здесь запрещены → 403 `attendance_correction_admin_only`; они только admin-side (§2.21). |
...
2. **Prepayment** — досрочная оплата на 3/6/12 месяцев (опционально 24 — если `discount_rules.prepay_24m_pct` задан админом). Создаётся отдельный invoice с `invoice_type='prepayment_{3|6|12|24}m'` и скидкой `prepay_{N}m_pct`. Окно — всегда с 1-го числа следующего месяца (Almaty-anchored); если будущие месяцы уже покрыты оплаченной предоплатой, окно сдвигается на первый непокрытый месяц. Сумма считается помесячно с вычетом заведённых нерабочих праздников (`цена × (дни − праздники) / дни`), итог квантуется до целых тенге; цена фиксируется на момент создания (поздние правки праздников/тарифа пересчёта не делают). При наличии долга (`pending`/`overdue`/`partial` не-prepayment инвойс ребёнка) создание блокируется (400 `prepayment_blocked_outstanding_debt`).
...
| GET | `/parent/invoices/:id/payment-methods` | Реально доступные способы оплаты для садика инвойса. Deployment allowlist (`PAYMENT_PROVIDERS`) пересекается с per-tenant состоянием: `bcc` возвращается только при `bcc_merchant_accounts.status='active'`, `kaspi_pay` — при active Kaspi session. Response: `{providers: [{provider, kind: 'redirect'\|'deeplink', display_name}]}`. Те же `InvoiceAccessGuard` и guardian permissions, что у `/pay`. |
| GET | `/parent/payment-profile` | Приватный provider-neutral billing profile текущего пользователя. Если сохранён: `{billing_phone, billing_address, saved: true}`; если нет: `{billing_phone: users.phone, billing_address: null, saved: false}`. Tenant admin не имеет доступа к этому route/репозиторию. |
| PUT | `/parent/payment-profile` | Атомарно сохранить либо заменить **оба** значения. Body: `{billing_phone, billing_address}`. Это не меняет `users.phone`; смена login phone остаётся отдельным OTP-flow. Response 200: `{billing_phone, billing_address, saved: true}`. Значения не логируются. |
| DELETE | `/parent/payment-profile` | Удалить оба сохранённых billing-реквизита. Следующий GET снова предзаполняет телефон из `users.phone`, адрес остаётся пустым. Response 204. |
| POST | `/parent/invoices/:id/pay` | Инициировать оплату текущего invoice. Body: `{provider: 'mock'\|'halyk_epay'\|'kaspi_pay'\|'tiptoppay'\|'freedom_pay'\|'bcc', payment_mode: 'full'\|'partial', amount?: 60000, idempotency_key: 'uuid-v4-client-generated', kaspi_phone_number?: '7XXXXXXXXXX', billing_phone?: '+7XXXXXXXXXX', billing_address?: '...', save_billing_profile?: false}`. **`idempotency_key` обязателен** — UUID, клиент генерирует per-attempt; повторный запрос с тем же ключом возвращает тот же `payment_id` и, пока сессия жива, тот же checkout URL. `payment_mode='full'` = «погасить **остаток**»: сервер сам считает сумму как `amount_after_discount − amount_paid` и игнорирует переданный `amount`, поэтому дооплата частично оплаченного invoice проходит штатно (до фикса подставлялась полная цена → 409 `invoice_status_invalid` / `amount_mismatch_full` на каждом `partial`). Если остаток уже ≤ 0 → 409 `invoice_already_paid`. При `partial` `amount` обязателен и должен быть > 0 и ≤ остатка invoice. **`partial` допустим только на месячном счёте:** предоплатный (`invoice_type='prepayment_*'`) неделим — скидка `prepay_{N}m_pct` даётся именно за оплату N месяцев одним платежом, поэтому `payment_mode='partial'` на нём → 400 `prepayment_partial_not_allowed` (`details: {invoice_id, invoice_type, amount_due}`). Для `kaspi_pay` обязателен `kaspi_phone_number`. Для `bcc` обязательны подтверждённые `billing_phone` и `billing_address`; при `save_billing_profile=true` оба атомарно upsert'ятся в `user_payment_profiles`, но `users.phone` не меняется. BCC создаёт `payments(status='processing')` и одноразовую Redis checkout-session TTL 15 минут, возвращая `{payment_id, redirect_url}` на Shyraq bridge. Карта/PAN/CVC в API не принимаются. BACKREF не означает успех: финальный статус задают только BCC callback/reconciliation. Требует active provider account сада; иначе 409 `bcc_not_connected` / `kaspi_not_connected`. **Tenant из ресурса:** `InvoiceAccessGuard` резолвит садик инвойса и `assertCanPay` проверяет guardian в нём. Доступен для `primary`/`secondary`; 403 для `nanny`. Errors: 400 `payment_provider_unavailable`, 400 `prepayment_partial_not_allowed`, 400 `kaspi_phone_required`, 400 `bcc_billing_details_required`, 404 `invoice_not_found`, 409 `invoice_already_paid`, 409 `bcc_not_connected`, 409 `kaspi_not_connected`, 409 `payment_idempotency_conflict`, 410 `bcc_checkout_expired`, 429 rate-limit. |
| POST | `/parent/invoices/:id/pay/prepayment` | Досрочная оплата. Body: `{months: 3\|6\|12\|24, provider, idempotency_key, billing_phone?, billing_address?, save_billing_profile?}`. BCC billing-поля имеют те же правила, что основной `/pay`. Сервер создаёт invoice `invoice_type='prepayment_{N}m'` по квоте: окно с 1-го числа следующего месяца, **сдвигается** вперёд на первый месяц, не покрытый оплаченной предоплатой; сумма помесячная с вычетом праздников, минус `prepay_{N}m_pct` и кастомные скидки, итог — целые тенге. **Line item на каждый покрываемый месяц** (quantity=1, `unit_price = line_total` = целотенговая доля месяца, description `Prepayment YYYY-MM — {план}`). Если долгов нет, но у ребёнка висит `pending`/`overdue` prepayment-инвойс — он **отменяется** и создаётся новый (`partial` prepayment не трогается); уведомление об отмене не шлётся. Инициирует платёж. Ответ: `{invoice_id, payment_id, redirect_url?, deeplink?, preview: {base_amount, discount_pct, final_amount, covers_period: {from, to}}}`. Доступ — как у основного `/pay` (через `assertCanPay`): `primary`, а также `secondary` при `permissions.invoice_pay !== false`; 403 для `nanny`. Errors: 400 `prepayment_blocked_outstanding_debt` (есть `pending`/`overdue`/`partial` не-prepayment долг; `details.outstanding_amount` — сумма), 400 `prepayment_blocked_partial_prepayment` (у ребёнка уже висит предоплата, по которой прошли деньги — такую НИКОГДА не отменяем автоматически; `details: {invoice_id, paid_amount}`), 400 `prepayment_blocked_window_overlap` (после сдвига окно накрывает месяцы, уже покрытые оплаченной предоплатой, т.е. покрытие вышло бы разрывным — иначе списали бы дважды за один месяц; `details: {covered_months}` — массив `YYYY-MM`), 400 `payment_provider_unavailable`, 400 `prepayment_horizon_not_configured`, 400 `bcc_billing_details_required`, 409 `bcc_not_connected`. |
| GET | `/parent/invoices/:id/prepayment-preview?months=3\|6\|12\|24` | Превью предоплаты: **тот же расчёт**, что у `/pay/prepayment`, но с **нулём записей** — не создаёт invoice, не резервирует capped custom-discount слоты, не отменяет старые pending-prepayment. Anchor — любой инвойс ребёнка; те же `InvoiceAccessGuard` (tenant из ресурса) и `assertCanPay`-права, что у pay-роутов. **Снапшот ответа 200 (SoT для фронта):** `{"blocked_reason": null, "outstanding_amount": null, "window": {"from": "2026-08-01", "to": "2026-10-31"}, "months": [{"period_start": "2026-08-01", "period_end": "2026-08-31", "base_amount": 60000, "holiday_days": 0, "amount_share": 54000}, {"period_start": "2026-09-01", "period_end": "2026-09-30", "base_amount": 60000, "holiday_days": 0, "amount_share": 54000}, {"period_start": "2026-10-01", "period_end": "2026-10-31", "base_amount": 60000, "holiday_days": 0, "amount_share": 54000}], "discount_pct": 10, "total": 162000}`. **Блокировки отдаются как HTTP 200, не 400** (в отличие от `/pay/prepayment`, который на тех же условиях кидает 400) — превью всегда отвечает телом, фронт смотрит на `blocked_reason`. Значений три: `outstanding_debt` (у ребёнка есть `pending`/`overdue`/`partial` не-prepayment долг → заполнен `outstanding_amount`), `partial_prepayment_exists` (уже висит предоплата, по которой прошли деньги → заполнены `blocked_invoice_id` и `blocked_paid_amount`; **внимание:** в 400-ке от `/pay/prepayment` те же два значения приходят под именами `details.invoice_id` / `details.paid_amount` — ключи намеренно разные, это не опечатка), `window_overlaps_covered` (после сдвига окно накрывает месяцы, уже покрытые оплаченной предоплатой → заполнен `covered_months`: массив ключей `YYYY-MM`). При любом из них `window`, `discount_pct`, `total` — `null`, а `months` — пустой массив. Пример долга: `{"blocked_reason": "outstanding_debt", "outstanding_amount": 60000, "window": null, "months": [], "discount_pct": null, "total": null}`. Семантика полей: `window` — покрываемое окно (с 1-го числа следующего месяца, Almaty; сдвинуто мимо месяцев, уже покрытых оплаченной предоплатой); `base_amount` — помесячная цена до скидки с вычетом праздников (`цена × (дни − праздники) / дни`, 2dp); `holiday_days` — вычтенные нерабочие дни месяца; `amount_share` — целотенговая доля месяца в `total` (Σ shares = total ровно; из них строятся line items при оплате); `discount_pct` — null, если скидка абсолютная (custom fixed-KZT); `total` — целые тенге, ровно та сумма, которую спишет `/pay/prepayment`. Errors: 400 `prepayment_horizon_not_configured`, 403 nanny/чужой, 404 `invoice_not_found`, 422 validation (months ∉ {3,6,12,24}). |
| GET | `/parent/children/:id/payment-calendar` | Календарь платежей в Kaspi-стиле. Параметры: `months_ahead=12` (1..24; вне диапазона — 400 `months_ahead_out_of_range`). Окно — с 1-го числа текущего месяца (Almaty), один элемент на месяц. **Response 200:** `{child_id, months_ahead, invoices: [{period_start: '2026-08-01', period_end: '2026-08-31', invoice_id: uuid\|null, projected_status: 'pending'\|'paid'\|'overdue'\|'partial'\|'projected'\|'refunded'\|'cancelled', amount_after_discount: number\|null, due_date: 'YYYY-MM-DD'\|null, is_projection: bool, holidays_affected: int, invoice_type: 'monthly'\|'prepayment_{3\|6\|12\|24}m'\|…\|null}]}`. Приоритет заполнения месяца: **(1) покрытие оплаченной предоплатой** — `invoice_id` предоплатного инвойса, `projected_status='paid'`, `is_projection=false`, `amount_after_discount` = целотенговая доля месяца из line item (legacy-предоплаты с одним line item — равномерный сплит), `invoice_type='prepayment_{N}m'` (фронт подписывает «оплачено предоплатой»); **(2) существующий invoice** за месяц (cron / fee / pending-prepayment) — реальные данные + его `invoice_type`; **(3) projection** из активного `tariff_assignments` + `kindergarten_holidays` (`invoice_id=null`, `invoice_type=null`, `due_date=null`, `projected_status='projected'`). Pending/partial предоплата НЕ размазывается — видна только в месяце своего `period_start`. Доступен для `primary`/`secondary`, **403 для `nanny`**. |
...
| 400 | `prepayment_blocked_outstanding_debt` | Создание предоплаты при наличии `pending`/`overdue`/`partial` не-prepayment инвойса ребёнка; `details.outstanding_amount` — сумма долга |
| 400 | `months_ahead_out_of_range` | `months_ahead` вне 1..24 в `/payment-calendar` |
| 400 | `bcc_billing_details_required` | Для BCC не переданы одновременно billing phone и address |
| 409 | `bcc_not_connected` | BCC account садика отсутствует, draft или disabled |
| 410 | `bcc_checkout_expired` | Одноразовая checkout-session истекла или уже использована |
| 400 | `prepayment_partial_not_allowed` | `payment_mode='partial'` на счёте `invoice_type='prepayment_*'` (либо admin cash-приём меньше остатка). Предоплата неделима; `details: {invoice_id, invoice_type, amount_due}` |

#### Родительские тексты ошибок предоплаты

API **не отдаёт человекочитаемых сообщений**: `DomainErrorFilter` кладёт в `message` тот же машинный код, что и в `error`. Текст собирает приложение — вот согласованные формулировки (RU) и поля `details`, из которых они строятся. Без них родитель видит голый код.

| `error` | Текст в приложении |
|---|---|
| `prepayment_blocked_outstanding_debt` | «Сначала погасите задолженность {`outstanding_amount`} ₸, затем оформите досрочную оплату.» |
| `prepayment_blocked_window_overlap` | «Досрочная оплата на {выбранный срок} месяцев недоступна: {`covered_months`, названиями месяцев} уже оплачены. Выберите меньший срок.» |
| `prepayment_blocked_partial_prepayment` | «У вас есть незавершённая досрочная оплата на {`paid_amount`} ₸. Обратитесь к администратору детского сада.» |
| `prepayment_partial_not_allowed` | «Досрочная оплата вносится одной суммой — {`amount_due`} ₸. Разделить её на части нельзя.» |
| `prepayment_horizon_not_configured` | «Досрочная оплата на этот срок пока недоступна в вашем саду.» |

`covered_months` — это месяцы окна, которые **уже оплачены** (не «не оплачены»): окно стартует с первого непокрытого месяца, а разрыв в покрытии — как правило след возврата — оставляет оплаченные месяцы дальше по окну. Поэтому действие для родителя — выбрать **меньший** срок, помещающийся в разрыв.

⚠️ Известный тупик: если разрыв короче 3 месяцев, ни один допустимый срок (3/6/12/24) в него не влезет и любой выбор вернёт `prepayment_blocked_window_overlap`. Родителю остаётся админ. Порог редкий (нужен возврат, оставивший дыру в 1–2 месяца), поэтому отдельного кода под него пока нет.

### 4.4a BCC Checkout Bridge

**Auth:** routes публичные, но checkout защищён криптографически случайным bearer token. Token живёт 15 минут, привязан к `payment_id + kindergarten_id`, потребляется атомарно один раз и не попадает в логи.
...
| 401 | `bcc_callback_unauthorized` | Неверный callback token или Basic Auth |
| 400 | `bcc_callback_invalid` | Не совпали merchant/terminal/order/amount либо обязательные callback-поля |
...
> **Шаблон ≠ то, что видит родитель.** `GET /admin/schedule/templates/:id` (§2.8) отдаёт *определение* недели, а этот эндпоинт — конкретные материализованные `activity_events`. Мост между ними — cron `schedule:auto-copy` (вс 23:00 Asia/Almaty), который проецирует шаблон на следующую неделю, плюс автоматическая пересборка при каждой мутации шаблона (§2.8 `week-snapshots/rebuild`). Неделя без snapshot'а событий не имеет вовсе — родитель увидит пустоту, даже если шаблон заполнен. Времена слотов (`time` без TZ) трактуются как локальное время садика (`Asia/Almaty`) и проецируются в UTC-инстанты.
