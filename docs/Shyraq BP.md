...
- **Audience-разделение Staff vs Admin (строгое).** `app=admin` принимает только роль `admin`; `app=staff` — `mentor`/`specialist`/`reception`. **Admin не заходит в Staff App, и наоборот** (роль фильтруется по `app`, `aud` зашивается в токен). Если у телефона нет роли под запрошенную аппку → 403 `no_role_for_app`. **Исключение — Parent App (open-registration):** для `app=parent` авторизация проходит всегда. Audience-фильтр выкидывает staff/admin-роли, и при пустом результате подставляется неявная `{role:'parent', kg:null}` — поэтому даже телефон, заведённый в админке как сотрудник/админ, может войти в Parent App, добавить ребёнка по ИИН и увидеть pending-approval список. Понижение staff/admin → parent безопасно; обратная защита (parent не получает staff/admin-токен) сохраняется.
...
6. Родитель выбирает способ оплаты.
    1. Досрочная оплата (prepayment) — 3 / 6 / 12 / 24 месяца *(скидка `prepay_{N}m_pct` тарифа; 24 — только если админ задал `prepay_24m_pct`)*
        1. Окно — всегда с 1-го числа следующего месяца (Almaty); текущий месяц предоплатой не покрывается (он оплачивается обычным monthly). Если будущие месяцы уже покрыты действующей оплаченной предоплатой — окно сдвигается вперёд на первый непокрытый месяц.
        2. Блок при долге: любой `pending`/`overdue`/`partial` не-prepayment инвойс ребёнка запрещает создание предоплаты (400 `prepayment_blocked_outstanding_debt`) до полного погашения. Неоплаченный `pending`/`overdue` prepayment долгом НЕ считается — при новой попытке он отменяется и создаётся новый.
        3. Сумма считается помесячно с вычетом заведённых нерабочих праздников (`цена × (дни месяца − праздники) / дни месяца`), затем минус `prepay_{N}m_pct` и кастомные скидки; итог квантуется до целых тенге. Цена фиксируется на момент создания — поздние правки праздников/тарифа пересчёта не делают.
    2. Частичная оплата
...
- **Покрытие предоплатой (prepayment coverage):** подавляет биллинг только ОПЛАЧЕННЫЙ (`status='paid'`) `prepayment_{N}m` инвойс; pending/partial не подавляет ничего. Три механики: **(1) подавление крона** — `billing:invoice-generate` скипает детей, чей период покрыт оплаченной предоплатой (один batch-запрос на садик, skip логируется и попадает в счётчик `skipped`); **(2) settlement-хук** — при зачислении полной оплаты предоплатного инвойса перекрытые НЕоплаченные (`pending`/`overdue`) monthly в окне авто-отменяются с уведомлением родителю (reason `covered_by_prepayment`), их capped custom-discount слоты освобождаются; перекрытый уже `paid`/`partial` monthly — warn-лог для ручного разбора, авто-рефанда нет; **(3) календарь** — оплаченная предоплата размазывается по месяцам окна: каждый покрытый месяц отдаётся строкой `paid` с `invoice_id` предоплатного инвойса, целотенговой долей месяца из line item и `invoice_type='prepayment_{N}m'`.
...
- Staff App
- Parent App
- Admin Web *(ведение журнала и корректировки — см. 5.5, 5.6)*
...
1. Mentor/Reception правят только время, заметку и того, кто забрал, и только в пределах текущего календарного дня (Asia/Almaty) — окно смены.
2. Admin правит запись любой давности и, дополнительно, исправляет **структурные ошибки**:
    1. Запись оформили не на того ребенка — перепривязка на правильного.
    2. Промахнулись кнопкой (вход вместо выхода) — переключение типа прохода.
    3. Прохода не было вовсе — удаление записи.
3. Система каскадно приводит в порядок производные данные: запись в `timeline` переезжает вместе с событием (или удаляется вместе с ним), внутридневной статус пересчитывается — у обоих детей, если событие перевесили.
4. Каждая правка журналируется: кто, когда, что было и что стало. История правок доступна админу по конкретному событию и переживает его удаление.
5. Способ фиксации (`face_id`/`manual`/`otp_pickup`) не редактируется — он фиксирует, как запись появилась.

### 5.6 Ведение журнала из админки

1. Ребенок прошел, но турникет/Staff App не зафиксировали (сбой, ребенка провели вручную, забыли отметить).
2. Reception или Admin регистрирует проход из Admin Web:
    1. По QR родителя/доверенного лица: скан определяет **только личность** и список детей — сам по себе он посещаемость не пишет; сотрудник выбирает ребенка и отдельным действием фиксирует вход или выход.
    2. Вручную, выбрав ребенка из списка.
3. Проход текущим днем — обычное событие: статус ребенка обновляется, родителю уходит уведомление (как в 5.1 / 5.4).
4. Проход **задним числом** (админ закрывает журнал за прошедший день) — событие, `timeline` и журнал правок пишутся, но **уведомление родителю не отправляется**: родитель не должен получать «ребенок пришел» вечером следующего дня.
5. Отдельно админ проставляет внутридневной статус на дату (`болеет`, `в отпуске`, `опоздал` и т.д.). Проставленный вручную статус — операторское решение и приоритетнее выведенного из проходов: последующий check-in или пересчет его не перетирают.

### Result

- Все ключевые события дня отражены в системе и видны родителю.
- Журнал посещаемости остается достоверным даже когда автоматика не сработала: пробел закрывается из админки, а каждая правка прослеживается до автора.
