...
export async function truncateAll(dataSource: DataSource): Promise<void> {
  ...
  await dataSource.transaction(async (m) => {
    ...
    await m.query(
      `TRUNCATE TABLE saas_refresh_tokens, saas_users, timeline_entries, attendance_events, child_daily_status, schedule_week_snapshots, activity_events, schedule_template_slots, schedule_templates, meal_items, meal_plans, child_group_history, child_guardians, children, group_mentors, groups, cameras, locations, staff_members, refresh_tokens, users, kindergartens RESTART IDENTITY CASCADE`,
    );
  });
}
