...
export async function truncateAll(dataSource: DataSource): Promise<void> {
  ...
  await dataSource.transaction(async (m) => {
    ...
    await m.query(
      `TRUNCATE TABLE saas_refresh_tokens, saas_users, group_mentors, groups, cameras, locations, staff_members, refresh_tokens, users, kindergartens RESTART IDENTITY CASCADE`,
    );
  });
}
