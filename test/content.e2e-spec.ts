  ...
  describe('Scenario I: Mentor lists own group stories; admin lists all', () => {
    it(
      ...
      async () => {
        ...
        // Admin creates story in group B
        await assignMentorToGroup(kgId, groupB, adminUserId);
        const adminMentorToken = await mintToken({
          sub: adminUserId,
          role: 'mentor',
          kindergartenId: kgId,
        });
        const storyBRes = await request(server)
          .post('/api/v1/staff/stories')
          .set('Authorization', `Bearer ${adminMentorToken}`)
