...
Все API (кроме `/super-admin/*`) работают через `KindergartenScopeGuard` — `kindergarten_id` берётся из JWT и автоматически применяется к запросам. Все запросы проходят через `RateLimitInterceptor` (Redis sliding window).
...
Email + password (не OTP). Access-токен — тот же JWT HS256 (`JWT_ACCESS_SECRET`, TTL 15m) с `role='super_admin'` (или `'support'`) и без `kindergarten_id`. Refresh-токен — opaque random hex 32 bytes, **хранится в отдельной таблице `saas_refresh_tokens`** (FK на `saas_users.id`, не на `users.id`). TTL тот же `REFRESH_TOKEN_TTL_DAYS`. Все `/super-admin/*` controllers помечаются декоратором `@SuperAdminScope()` (D6) — `KindergartenScopeGuard` через metadata разрешает выполнение без `kindergarten_id` и активирует Prisma-bypass.

| Метод | Путь | Назначение |
|---|---|---|
| POST | `/super-admin/auth/login` | Публичный (`@Public()`). Body `{email, password}`. Lookup `saas_users WHERE email=? AND is_active=true`. `bcrypt.compare`; любой негативный исход (user не найден, wrong password, `is_active=false`) → 401 `invalid_credentials` без утечки причины. Rate-limit 10/hour per email. При успехе — issue JWT `{sub, role, jti}` (без `kindergarten_id`, без `pending_role_select`) + opaque refresh в `saas_refresh_tokens`. Response идентичен parent-auth-response §0.1 минус `kindergartens[]` (всегда пустой). |
| POST | `/super-admin/auth/refresh` | Аналогично `/auth/refresh`, но работает против `saas_refresh_tokens`. Error codes идентичны. Клиент должен передать текущий access-токен в `Authorization: Bearer …` вместе с body refresh_token — без него blocklist пропускается (best-effort). |
| POST | `/super-admin/auth/logout` | Аналогично `/auth/logout`, но ревокирует запись в `saas_refresh_tokens` + добавляет access-`jti` в общий `token:blocklist:{jti}`. |

### 1.2 Kindergartens (Tenants)

| Метод | Путь | Назначение |
|---|---|---|
| GET | `/super-admin/kindergartens` | Список всех садиков (с фильтрами: `plan`, `is_active`, поиск по имени). |
| POST | `/super-admin/kindergartens` | Атомарное создание tenant'а. Body: `{name, slug, address?, phone?, plan?, settings?, admin: {full_name, phone, locale?}}`. В одной Prisma-транзакции: `kindergartens` insert + find-or-create `users` по `admin.phone` (если phone уже существует — переиспользуем `user_id`, имя/locale не меняем) + `staff_members` (role=`admin`, `is_active=true`). После commit — `SmsPort.send` welcome-SMS (best-effort, log-on-fail, транзакцию не откатывает). Response 201 `{kindergarten, staff_member, user}`. Ошибки: 400 (invalid slug/phone format), 409 `kindergarten_slug_taken`, 422 (validation). **Активация admin'а = welcome-SMS + обычный B1 OTP-flow** (см. §0.1 `/auth/otp/request`+`/auth/otp/verify`); отдельного invite-token / magic-link нет. |
| GET | `/super-admin/kindergartens/:id` | Подробности садика: настройки, подписка, статистика (кол-во детей, активных подписок). |
| PATCH | `/super-admin/kindergartens/:id` | Обновить `settings` (timezone, currency, late_pickup_fee_amount, otp_expiry_seconds, prepay скидки, payment_grace_days, fiscal-конфиг), `plan`, `is_active`. |
| DELETE | `/super-admin/kindergartens/:id` | Soft-delete (через `is_active=false`), cascade-архивация активных сущностей. |

### 1.3 SaaS Subscriptions

| Метод | Путь | Назначение |
|---|---|---|
| GET | `/super-admin/saas-subscriptions` | Список подписок платформы по садикам. |
| POST | `/super-admin/saas-subscriptions` | Создать подписку (`plan_code`, `billing_period`, `amount`, `started_at`). |
| PATCH | `/super-admin/saas-subscriptions/:id` | Изменить статус (`active`/`trial`/`suspended`/`cancelled`), `next_billing_at`. |

### 1.4 Feature Flags

| Метод | Путь | Назначение |
|---|---|---|
| GET | `/super-admin/feature-flags` | Все флаги (глобальные + per-tenant). Фильтр по `kindergarten_id`. |
| POST | `/super-admin/feature-flags` | Создать/обновить флаг (`key`, `value` JSONB, опционально `kindergarten_id=null` — глобальный). |
| DELETE | `/super-admin/feature-flags/:id` | Удалить флаг. |

### 1.5 SaaS Users

| Метод | Путь | Назначение |
|---|---|---|
| GET | `/super-admin/users` | Список SaaS-пользователей (super_admin, support). |
| POST | `/super-admin/users` | Создать SaaS-пользователя. |
| PATCH | `/super-admin/users/:id` | Деактивация, смена роли. |
...
| Метод | Путь | Назначение |
|---|---|---|
| GET | `/parent/children` | Привязанные дети (approved). **[Deferred to B6 — Parent App Onboarding]** |
| POST | `/parent/children/link` | Привязка ребёнка по ИИН: ищет `children` в tenant'ах → если найдено → `child_guardians (role='nanny'|'secondary', status='pending_approval')`. Если родитель — Primary Guardian из enrollment (по телефону) → авто-approve. **[Deferred to B6]** |
| POST | `/parent/children/:id/unlink` | Отвязать (soft-revoke — `revoked_at`). **[Deferred to B6]** |
| GET | `/parent/children/:id` | Полная карточка: mentor, группа, timeline, события дня, оплата, диагностики, прогресс. **[Deferred to B6 — полная карточка с timeline/payments]** |
