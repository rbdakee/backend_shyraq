...
describeIntegration(
  ...
  () => {
    ...
    /**
     * Force an existing invoice into a terminal-ish status the service layer
     * cannot reach directly (`partial` / `overdue`), optionally attaching a
     * completed payment so the row genuinely holds money. Raw SQL on purpose:
     * these are states the nightly crons produce, and the blocking rules must
     * hold against the real column values, not a service-shaped fake.
     */
    async function forceInvoiceStateRaw(
      invoiceId: string,
      status: 'partial' | 'overdue',
      opts: { kgId: string; childId: string; paidKzt?: number } | null = null,
    ): Promise<void> {
      await dataSource.transaction(async (m) => {
        await m.query(`SET LOCAL app.bypass_rls = 'true'`);
        ...
        if (opts?.paidKzt) {
          await m.query(
            `INSERT INTO payments
               (id, kindergarten_id, invoice_id, child_id, amount, provider,
                provider_txn_id, idempotency_key, status, paid_at)
            ...
            [
              ...
              opts.childId,
              opts.paidKzt,
              `tx-partial-${randomUUID()}`,
              `partial-int-${randomUUID()}`,
            ],
          );
        }
      });
    ...
    // ── Scenario 0: creation-blocking rules against real column values ────

    it('blocks prepayment creation on a partially paid monthly — partial counts as debt, not as settled', async () => {
      const seed = await seedKindergarten(1);
      const [childId] = seed.childIds;
      try {
        const h = makeHarness();
        const monthly = await seedPendingMonthly(
          seed.kgId,
          childId,
          seed.accountIds[0],
          seed.planId,
        ...
        // Half paid — the parent still owes the remainder, so a prepayment
        // must not jump the queue ahead of the outstanding month.
        await forceInvoiceStateRaw(monthly, 'partial', {
          kgId: seed.kgId,
          childId,
          paidKzt: MONTHLY_AMOUNT / 2,
        });

        await expect(
          inTenantTx(seed.kgId, () =>
            h.invoiceService.prepayInvoice(seed.kgId, childId, 3),
          ),
        ).rejects.toBeInstanceOf(PrepaymentBlockedOutstandingDebtError);
      } finally {
        await seed.cleanup();
      }
    });

    it('blocks a new prepayment behind a money-holding stale one and leaves that row untouched', async () => {
      const seed = await seedKindergarten(1);
      const [childId] = seed.childIds;
      try {
        const h = makeHarness();
        const stale = await inTenantTx(seed.kgId, () =>
          h.invoiceService.prepayInvoice(seed.kgId, childId, 3),
        );
        // `markOverdueBatch` collapses a part-paid prepayment to `overdue`,
        // so the money-holding check must key on the payment sum, not on the
        // status — an `overdue` row can still be holding the parent's cash.
        await forceInvoiceStateRaw(stale.id, 'overdue', {
          kgId: seed.kgId,
          childId,
          paidKzt: 1_000,
        });
        ...
        ).rejects.toBeInstanceOf(PrepaymentBlockedPartialPrepaymentError);

        // The blocked path must never auto-cancel a row that holds money —
        // cancelling it would strand the parent's payment.
        expect(await readInvoiceStatus(stale.id)).toBe('overdue');
      } finally {
        await seed.cleanup();
      }
    });

    it('cancels an OVERDUE covered monthly when the prepayment settles, not just a pending one', async () => {
      const seed = await seedKindergarten(1);
      const [childId] = seed.childIds;
      try {
        const h = makeHarness();
        ...
        const covered = await seedPendingMonthly(
          seed.kgId,
          childId,
          seed.accountIds[0],
          seed.planId,
        ...
        // The nightly overdue cron ran before the parent's payment landed.
        await forceInvoiceStateRaw(covered, 'overdue');

        const txnId = `tx-overdue-cancel-${randomUUID()}`;
        await seedProcessingPayment(
          seed.kgId,
          prepayment.id,
          childId,
          prepayment.amountAfterDiscount.toNumber(),
          txnId,
        );
        ...
        await h.paymentService.processWebhook({
          provider: 'mock',
          headers: {},
          body: { provider_txn_id: txnId, status: 'completed' },
        });

        expect(await readInvoiceStatus(prepayment.id)).toBe('paid');
        // Pre-fix the cancel path accepted only `pending` source rows, so an
        // overdue covered month survived and kept dunning the parent for a
        // month they had just prepaid.
        expect(await readInvoiceStatus(covered)).toBe('cancelled');
      } finally {
        await seed.cleanup();
      }
    });
